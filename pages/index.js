...
export default function Home() {
  ...
  const [showFergusonForm, setShowFergusonForm] = useState(false);
  ...
  const handleEmailReport = async () => {
    ...
    if (!constituentName.trim() || !constituentEmail.trim()) { setEmailStatus('❌ Name and email required'); setTimeout(() => setEmailStatus(''), 4000); return; }
              ...
              {result.error ? <p style={{ color:'#dc2626', fontSize:15 }}>{result.error}</p> : (
                <>
                  ...
                  <div style={{ marginTop:16, display:'flex', gap:8, flexWrap:'wrap', alignItems:'center' }}>
                    ...
                    {showFergusonForm && (
                      <div style={{ width:'100%', backgroundColor:'#f0f4ff', border:'1px solid #c7d9fa', borderRadius:8, padding:'12px 14px', marginTop:4 }}>
                        <p style={{ margin:'0 0 8px 0', fontSize:12, fontWeight:600, color:'#1a3a5c' }}>Your contact info (so Councilman Ferguson can follow up):</p>
                        <div style={{ display:'flex', gap:8, flexWrap:'wrap', marginBottom:8 }}>
                          <input type="text" value={constituentName} onChange={e => setConstituentName(e.target.value)} placeholder="Your name (required)"
                            style={{ flex:1, minWidth:160, padding:'8px 10px', fontSize:13, border:'1px solid #c7d9fa', borderRadius:6, outline:'none', fontFamily:'inherit' }} />
                          <input type="email" value={constituentEmail} onChange={e => setConstituentEmail(e.target.value)} placeholder="Your email (required)"
                            style={{ flex:1, minWidth:160, padding:'8px 10px', fontSize:13, border:'1px solid #c7d9fa', borderRadius:6, outline:'none', fontFamily:'inherit' }} />
                          <input type="tel" value={constituentPhone} onChange={e => setConstituentPhone(e.target.value)} placeholder="Phone (optional)"
                            style={{ flex:1, minWidth:140, padding:'8px 10px', fontSize:13, border:'1px solid #c7d9fa', borderRadius:6, outline:'none', fontFamily:'inherit' }} />
                        </div>
                        <button onClick={handleEmailReport} disabled={emailing || constituentName.trim() === '' || constituentEmail.trim() === ''}
                          style={{ backgroundColor:'#1a3a5c', color:'white', border:'none', padding:'8px 14px', fontSize:12, fontWeight:600, borderRadius:6, cursor: (emailing || constituentName.trim() === '' || constituentEmail.trim() === '') ? 'not-allowed' : 'pointer' }}>
                          {emailing ? '⏳ Sending...' : 'Send to Councilman Ferguson →'}
                        </button>
                      </div>
                    )}
                  </div>
