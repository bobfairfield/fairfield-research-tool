...
export default async function handler(req, res) {
  ...
  try {
    ...
    const commentSection = comment ? `
      <div style="background:#fef9e7;padding:20px 32px;border-left:4px solid #f39c12;margin-bottom:0">
        <p style="margin:0;font-size:13px;color:#666;font-weight:600;text-transform:uppercase;letter-spacing:1px">Note from Constituent</p>
    ...
    const summarySection = summary ? `
      <div style="background:#eef6ff;padding:20px 32px;border-left:4px solid #1a3a5c;margin-bottom:0">
        ...
        <div style="margin:8px 0 0 0;font-size:15px;color:#374151;white-space:pre-wrap">${summary}</div>
      </div>` : '';
    ...
    const { data, error } = await resend.emails.send({
      ...
      html: `<div style="font-family:Georgia,serif;max-width:680px;margin:0 auto;color:#1a1a2e">
        <div style="background:#1a3a5c;padding:24px 32px;border-radius:8px 8px 0 0">
          <h1 style="color:white;margin:0;font-size:20px">Fairfield Civic Intelligence Hub</h1>
          <p style="color:#a8c4e0;margin:4px 0 0 0;font-size:13px">${modeLabel} · ${date}</p>
        </div>
        <div style="background:#f8fafc;padding:20px 32px;border-left:4px solid #1a3a5c">
          <p style="margin:0;font-size:13px;color:#666;font-weight:600;text-transform:uppercase;letter-spacing:1px">Question</p>
        ...
        <div style="background:white;padding:28px 32px;border:1px solid #e2e8f0;border-top:none;border-radius:0 0 8px 8px">
          ...
          <div style="font-size:15px;line-height:1.8;color:#374151;white-space:pre-wrap">${analysis}</div>
        </div>
        <p style="text-align:center;font-size:11px;color:#aaa;margin-top:20px">Fairfield & Jefferson County Civic Intelligence Hub · civicintelligencehub.com</p>
      </div>`
    });
